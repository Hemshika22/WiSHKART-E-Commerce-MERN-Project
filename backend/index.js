...
mongoose.connect("mongodb+srv://hemshikagautamc:gG5XxlSgrHvCqXoH@cluster0.tezisg6.mongodb.net/e-commerce");
...
res.json({
 ...
 //CreatingEndpoint for registering the user
app.post('/signup', async (req,res)=>{

    let check = await Users.findOne({email: req.body.email}) 
    if(check) {
        return res.status(400).json({success:false, errors:'existing user with this email'});
    }
    let cart = {};
     for (let i = 0; i < 300; i++){
        cart[i] = 0;
    ...
    app.post('/login', async (req,res)=>{
        ...
        //creating middleware to fetch data
